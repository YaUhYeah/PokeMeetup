...
public class BattleAssets {
    private static final String TAG = "BattleAssets";
    private static final String ATLAS_PATH = "atlas/ui-gfx-atlas.atlas";
    public static final int HP_BAR_HEIGHT = 16;
    private static final int STATUS_ICON_WIDTH = 32;
    ...
    public void initialize() {
        try {
            // Load atlas directly from correct path
            ...
            GameLogger.info( "Battle assets initialized successfully");
    ...
    private void loadDataboxes() {
        databoxes = new HashMap<>();
        TextureRegion normal = uiAtlas.findRegion("info-box-bg");
        TextureRegion foe = uiAtlas.findRegion("info-box-bg");
    ...
    public void initialize(TextureAtlas uiAtlas) {
        this.uiAtlas = uiAtlas;

        // Load battle backgrounds
        TextureRegion bgRegion = uiAtlas.findRegion("battle-menu-bg");
        if (bgRegion != null) {
            battleBackground = new TextureRegionDrawable(bgRegion);
            menuBackground = new TextureRegionDrawable(uiAtlas.findRegion("window"));
        } else {
            GameLogger.error("Failed to load battle backgrounds");
        ...
        TextureRegion buttonSheet = uiAtlas.findRegion("battle-buttons");
        if (buttonSheet != null) {
            TextureRegion[][] buttons = buttonSheet.split(192, 48);
            buttonNormal = new TextureRegionDrawable(buttons[0][0]);
            buttonPressed = new TextureRegionDrawable(buttons[0][1]);
        } else {
            GameLogger.error("Failed to load button styles");
    ...
    private Color getHPColor(float percentage) {
        if (percentage > 0.5f) return HP_HIGH;
        if (percentage > 0.25f) return HP_MED;
        return HP_LOW;
    }
    ...
    public TextureRegionDrawable getBattleBackground() { return battleBackground; }
    public TextureRegionDrawable getMenuBackground() { return menuBackground; }
    public TextureRegionDrawable getButtonNormal() { return buttonNormal; }
    public TextureRegionDrawable getButtonPressed() { return buttonPressed; }
